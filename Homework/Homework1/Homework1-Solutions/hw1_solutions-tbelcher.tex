...
\item \emph{(Question 4)} Codex assisted me with this problem. I first began by investigating the data. Since there are no headers, I asked Codex to write some code that would give an eumeration of values or at least summary statistics in each category. 
Although quite a few features turned out to be binary, there were several that were not. After reviewing the models available from scikit, it seemed to me that since not all of the features were binary or clearly all the same kind of feature, I could not use one of the
specialized models that made restrictive assumptions (like BernoulliNB for binary features). Since GuassianNB seems the most general and therefore appropriate for the data, I ran that, but Codex suggested we could also look at Categorical after binning the data (and possibly losing numeric features). I let Coded run both after the Gaussian 
just to see if there was any improvement.\\

Results from the code are reproduced in the tables below. For 5 misclassified points, I was unsure where to start to identify why Bayes might have misclassified them. Given that so many features are binary I thought that perhaps the discrete nature of a binary category means it is easy for a small difference to flip the indicator.
I notice that Feature 10 for nearly all the misclassified points has values in the 30s. Perhaps looking at all points with those values and seeing how many are misclassified would produce some insights. I would also be interested in running kNN on the misclassified points just to compare them to their neighbors. 
\par\medskip
\noindent\begin{minipage}{\linewidth}
\textbf{GaussianNB results.}
The scikit Gaussian implementation in \path{Q4.ipynb} produced the following results.

\begin{center}
\begin{tabular}{lrrr}
\hline
Dataset & Rows & Input features & Total columns \\
\hline
Training & 700 & 24 & 25 \\
Test & 300 & 24 & 25 \\
\hline
...
\begin{tabular}{lrr}
\hline
Actual class & Predicted class 1 & Predicted class 2 \\
\hline
Class 1 & 162 & 45 \\
Class 2 & 33 & 60 \\
\hline
...
\noindent\textbf{Misclassified test examples.}
\begin{center}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{rrrp{0.62\linewidth}}
\hline
Test row & Actual & Predicted & Encoded features (F1--F24) \\
\hline
1 & 2 & 1 & \texttt{4, 12, 2, 11, 3, 3, 2, 4, 3, 29, 3, 1,}\newline
\texttt{1, 1, 1, 0, 0, 1, 0, 1, 0, 0, 1, 0} \\
\hline
2 & 2 & 1 & \texttt{1, 48, 4, 63, 1, 5, 3, 4, 4, 46, 3, 2,}\newline
\texttt{1, 2, 1, 0, 1, 1, 0, 0, 0, 0, 0, 1} \\
\hline
5 & 1 & 2 & \texttt{2, 27, 2, 25, 1, 2, 2, 1, 2, 32, 3, 1,}\newline
\texttt{2, 2, 1, 0, 0, 1, 0, 0, 1, 0, 0, 1} \\
\hline
21 & 2 & 1 & \texttt{3, 9, 0, 13, 1, 2, 3, 2, 3, 34, 3, 2,}\newline
\texttt{1, 2, 1, 0, 0, 1, 0, 0, 1, 0, 0, 0} \\
\hline
31 & 1 & 2 & \texttt{2, 24, 3, 64, 1, 2, 3, 2, 3, 33, 3, 1,}\newline
\texttt{1, 1, 1, 0, 0, 1, 0, 0, 1, 0, 0, 1} \\
\hline
\end{tabular}
\end{center}
...
\clearpage
\noindent\textbf{Corrected model results.}
Both models use the same 462 test examples from \path{Q1_test.csv}, with
standardization fitted only on training features. KNN uses $K=1$, selected
from $K=1,\ldots,9$ using a stratified validation split within the training
set. All nine values tied on validation macro-F1; the stated selection rule
favored the smallest $K$. Gaussian Naive Bayes estimates a Gaussian density
for each feature within each class and uses training class frequencies as
priors. Its Gaussian and conditional-independence assumptions are approximations
for the four numeric features.
...
\hline
Corrected KNN ($K=1$) & 0.998 & 0.998 \\
Gaussian Naive Bayes & 0.874 & 0.871 \\
\hline
\end{tabular}
\end{center}

\noindent Confusion matrices below use rows for true classes and columns for
predicted classes. Entries are counts; macro-F1 averages the F1 scores of
classes 0 and 1 equally.

\begin{center}
\begin{minipage}{0.48\linewidth}
\centering
\textbf{Corrected KNN ($K=1$)}\par\medskip
\begin{tabular}{lrr}
\hline
...
\begin{minipage}{0.48\linewidth}
\centering
\textbf{Gaussian Naive Bayes}\par\medskip
\begin{tabular}{lrr}
\hline
 & Predicted 0 & Predicted 1 \\
\hline
...
\noindent Results are reproduced from the executed notebook
\path{q5_corrected.ipynb} and its saved output
\path{q5_corrected_output.txt}.

\textbf{Reflection}. Repeating the same data as Q1 helped with this problem because I already had an idea what to expect. My python syntax knowledge is a bit sparse, especially for syntactic sugar on numpy functions, but
I was able to notice the basic issues with the wrong metric, including the class label in the point, and setting parameters using test values. The other issues Codex pointed out to me, although I feel as though I should have attempted to run
the code before asking it to analyze, just like I read the code before asking Codex to analyze it.\\

The code does attempt to normalize the data and handle a 0 variance case. As pointed out in the responses to the earlier bullet points, there were issues with attempting to read the header, including the class label, using the test points in parameter training, and not explicitly deciding what to do in the case
of a tie among neighbor votes. Looking at the results, we see that kNN does beter than Naive Bayes. The additional investigation I asked Codex to conduct to see if not making the naive assumption about independence among features would improve the result gave the 
expected result. Between Naive Bayes and kNN I would recommend using kNN given how much better it performs on accuracy and F1. Improving Bayes by investigating dependence between features is not something I would want to try if I did not have Codex to work through that code for me to test it.\\

As stated before, Codex GPT-6 Astra assisted me with this problem. Codex kept a log of our conversation and work on Problem 5 and that log can be viewed in the included Markdown file named \path{q5_ai_conversation_log.md}. 
% Student will write the required reflection separately.
